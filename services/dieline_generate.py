...
class DielineGenerator:
    def __init__(self, api_key: str = None):
        ...
        if not self.api_key:
            raise ValueError("GEMINI_API_KEY is required and not provided.")
        
        self.client = genai.Client(api_key=self.api_key)
        self.model_id = "gemini-2.0-flash"  # Using a standard model ID or the one specified in original code

        # To match the original code's specific model, we can try to use it if accessible
        # The original code used "gemini-3-pro-image-preview", assuming that's valid in user's context.
        # But commonly available is gemini-pro-vision or gemini-1.5-pro etc.
        # Keeping user's original model ID if possible, or defaulting to something safe.
        # The user's code had: MODEL_ID = "gemini-3-pro-image-preview" which might be a preview model.
        # I will keep it as literal string from original code to avoid breaking their specific access.
    ...
    def generate(self, dieline_path: str, concept_path: str, output_path: str):
        ...
        if img_cv is None: 
            print(f">>> [오류] Dieline 이미지를 읽을 수 없습니다: {dieline_path}")
            return
        ...
        try:
            ...
            print(f">>> [오류] Concept 이미지를 읽을 수 없습니다: {concept_path} ({e})")
            return
        ...
        if len(rooms) < 2:
            ...
            # Depending on requirements, we might want to raise an error or just return
            # raising error might stop the worker properly
            raise RuntimeError("패널을 충분히 찾지 못했습니다.")
        ...
        """
        bg_sheet = self._generate_target_image(bg_prompt, concept_img, w, h)
        if bg_sheet is None:
        ...
        if side1:
            ...
            side_bg_sq, (pad_x, pad_y, target_w, target_h) = self._make_square_canvas(side_bg_ref, fill_color=(0, 0, 0, 255))
        ...
        if top_panel:
            ...
            # Uses helper _make_square_canvas.

            top_bg_ref_sq, (pad_x, pad_y, target_w, target_h) = self._make_square_canvas(top_bg_ref, fill_color=(0, 0, 0, 255))
        ...
        bg_ref_sq, (pad_x, pad_y, target_w, target_h) = self._make_square_canvas(bg_ref, fill_color=(0, 0, 0, 255))
        ...
        **CRITICAL NEGATIVE CONSTRAINTS:**
                ...
                for part in response.candidates[0].content.parts:
                    if part.inline_data:
                        ...
                        if bbox:
                            ...
                            target_w_scaled = int(panel_w * 0.95)
                            target_h_scaled = int(panel_h * 0.95)
                            
                            cw, ch = cropped_art.size
                            
                            # Calculate scale to fit within target
                            scale = min(target_w_scaled / cw, target_h_scaled / ch)
                            new_w = int(cw * scale)
                            new_h = int(ch * scale)
        ...
        # Optional: Open file (disable in worker mode or enable only local)
        # if platform.system() == "Darwin": subprocess.run(["open", output_path])
        # elif platform.system() == "Windows": os.startfile(output_path)
...
if __name__ == "__main__":
    ...
    # Simple direct run test setup
    dieline_input = "dieline_input.png"
    concept_input = "concept_input.jpg"
    ...
    if os.path.exists(dieline_input) and os.path.exists(concept_input):
        print("Running DielineGenerator in interactive mode...")
        # API KEY logic for standalone run
        # Hardcoding a placeholder or expecting env var for standalone run
        # Assuming user has env var set or fills this in for local test
        api_key = os.environ.get("GEMINI_API_KEY")
        if not api_key:
             # Fallback to the hardcoded key from original file if they want, 
             # but better to ask user to set env.
        ...
        print(f"Usage: python dieline_generate.py [dieline_path] [concept_path]")
        print(f"Files not found: {dieline_input}, {concept_input}")